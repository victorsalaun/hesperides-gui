...
(function () {
    ...
    FileService.$inject = ['$hesperidesHttp', 'FileEntry', '$translate'];

    /* @ngInject */
    function FileService($http, FileEntry, $translate) {
        var service = {
            get_files_entries: get_files_entries,
        ...
        function files_rights_to_string(filesRights) {
        ...
        // Gets file name
        function get_file_name(location) {
            var tabs = location.split("/");
            return tabs[tabs.length - 1];
        }

        function get_files_entries(application_name, platform_name, path, module_name, module_version, instance_name, is_working_copy, simulate) {
        ...
        function download_files(entries, name) {
            // The JSZip Object
            var zip = new JSZip();
